...
void loop()
{
	...
	// WiFi Setup stays running all of the time until you have configures a WiFi router to connect SQUiXL to. 
	// You can configure the credentials any time, regardless of leaving the "first time" screen.
	...
	else if (wifi_controller.is_connected())
	{
		...
		{
			// We want to block the web server from starting if we have starting requests in the queue that are waiting to be processed
		...
		// We only process MQTT stuff if it's been enabled by the user
