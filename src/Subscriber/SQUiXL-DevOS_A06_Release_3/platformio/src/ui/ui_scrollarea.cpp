...
bool ui_scrollarea::redraw(uint8_t fade_amount, int8_t tab_group)
{
	...
	if (mqtt_stuff.mqtt_dirty || is_dragging || acceleration_y != 0)
	{
		...
		_sprite_content.setFreeFont(UbuntuMono_R[3]);  // Line added by @PaulskPt
		_sprite_content.setTextColor(TFT_ORANGE, -1);  // same
		
#ifdef USE_PAULSKPT_PARTS
#ifdef MY_DEBUG
		Serial.print(F("ui_scrollarea::redraw(): _x,_y _w,_h = "));
		Serial.printf("%d, %d  %d, %d\n", _x,_y,_w,_h);
#endif
		int y_inc = 22;  // Added by @Paulskpt for char size 3 y_inc 22. for size 1 y_inc 18
		int _scroll_h = 5;
		for (auto &sensor_data : mqtt_stuff.mqtt_topic_payloads)
		{
			line_y += 9;        // Added by @PaulskPt. Need some space from top
			psram_string heading = psram_string("At:") + sensor_data.second[0].timestampStr;
			_sprite_content.setCursor(_scroll_h, _scroll_y + line_y);
			_sprite_content.print(heading.c_str());
			line_y += y_inc; 
			psram_string msgID = psram_string("msgID:  ") +
				sensor_data.second[0].msgID;
			_sprite_content.setCursor(_scroll_h, _scroll_y + line_y);
			_sprite_content.print(msgID.c_str());
			line_y += y_inc;
#ifdef MY_DEBUG
			Serial.print("Owner = ");
			Serial.println(sensor_data.second[0].owner.c_str());
#endif
			psram_string owner = psram_string("Owner:  ") +
				sensor_data.second[0].owner;
			_sprite_content.setCursor(_scroll_h, _scroll_y + line_y);
			_sprite_content.print(owner.c_str());
			line_y += y_inc;

			std::string description;
			if (sensor_data.second[0].description == "Ext")
			  description = "External";
			else 
				description = sensor_data.second[0].description;
      ...
      if (sensor_data.second[0].owner == "Feath" ||
			...
			if (sensor_data.second[0].device_class == "wx")
				device_class = "weather";
			else
				device_class = sensor_data.second[0].device_class;
			
			std::string loc = "Loc:    " + description;

			_sprite_content.setCursor(_scroll_h, _scroll_y + line_y);
			_sprite_content.print(loc.c_str());
			
			line_y += y_inc;

			std::string device = dataType + device_class;
			_sprite_content.setCursor(_scroll_h, _scroll_y + line_y);
			_sprite_content.print(device.c_str());

			line_y += (5 * y_inc); //  was 2 * y_inc

			content_height += 5 * y_inc;  // Still needed for layout. Added by @PaulskPt

			int term_width = 0;
			for (const auto& reading : sensor_data.second) {
				if (reading.term.length() > term_width) {
						term_width = reading.term.length();
				}
			}
			term_width += 1;  // Add padding
		...
		_sprite_right.fillRect(20, 4, _scroll_h, _h - 56, static_cast<ui_screen *>(get_ui_parent())->dark_tint[2]);

		if (content_size_perc > 1.0f)
		{
			// content is shorter than content area height
			_sprite_right.fillRect(20, 4, _scroll_h, _h - 56, static_cast<ui_screen *>(get_ui_parent())->dark_tint[1]);
		}
		else
		{
			...
			_sprite_right.fillRect(20, 4 + scroll_y_pos, _scroll_h, len, static_cast<ui_screen *>(get_ui_parent())->dark_tint[1]);
		}
