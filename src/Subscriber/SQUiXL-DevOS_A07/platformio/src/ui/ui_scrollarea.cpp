...
bool ui_scrollarea::redraw(uint8_t fade_amount, int8_t tab_group)
{
	...
	{
		_sprite_content.fillScreen(static_cast<ui_screen *>(get_ui_parent())->dark_tint[2]);
		// Addition for test by @PaulskPt
		is_dragging = false;  // Reset dragging state to false, so we can redraw the content
		// --- end addition for test by @PaulskPt ---
		//_scroll_y
		...
		for (auto &sensor_data : mqtt_stuff.mqtt_topic_payloads)
		{
			...
			std::string heading = "At:" +                         // same
					sensor_data.second[0].timestampStr;           // same
			_sprite_content.setCursor(10, _scroll_y + line_y);    // same
			_sprite_content.print(heading.c_str());               // same
			line_y += y_inc;                                      // same

			std::string msgID = "msgID: " +                       // same
				sensor_data.second[0].msgID;                      // same
			_sprite_content.setCursor(10, _scroll_y + line_y);    // same
			_sprite_content.print(msgID.c_str());                 // same
			line_y += y_inc;                                      // same

			std::string owner = "Owner: " +                       // same
				sensor_data.second[0].owner;                      // same
			_sprite_content.setCursor(10, _scroll_y + line_y);    // same
			_sprite_content.print(owner.c_str());                 // same
			line_y += y_inc;                                      // same

			std::string locAndDevice = "Loc: " +                  // same
					sensor_data.second[0].description +           // same
					", Sensor: " +                                // same
					sensor_data.second[0].device_class;           // same 
			_sprite_content.setCursor(10, _scroll_y + line_y);    // same
			_sprite_content.print(locAndDevice.c_str());          // same
			line_y += (2 * y_inc);                                // same
			...
			for (int s = 0; s < sensor_data.second.size(); s++)
			{
				...
				oss << std::left  << std::setw(term_width) << reading.term
		...
		for (auto &sensor_data : mqtt_stuff.mqtt_topic_payloads)
		{
			for (int s = 0; s < sensor_data.second.size(); s++)
			{
				std::string data = sensor_data.second[s].description + " " + 
